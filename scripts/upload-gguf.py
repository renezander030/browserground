"""Per-file upload of _gguf-build/ → renezander030/browserground-gguf with retries."""
...
def main():
    ...
    gguf_dir = Path.home() / "funshorts" / "tools" / "imgparse-publish" / "_gguf-build"
    ...
    files = sorted([p for p in gguf_dir.rglob("*") if p.is_file()])
    print(f"[upload] {len(files)} files in {gguf_dir} → {repo_id}", flush=True)

    for f in files:
        rel = f.relative_to(gguf_dir).as_posix()
